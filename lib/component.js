...
export default function Component(oname, subscribe) {
  ...
  component = {
    ...
    component(name) {
      ...
      const mixinComponent = Component(name, component);
      const error = mixin(component, mixinComponent.$emitData);

      if (error) {
        const mixinError = new Error(`${mixinComponent.$name}组件与${$name}订阅组件之间存在属性合并冲突，$${error}`);
