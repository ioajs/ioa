...
const { apps, cwd, version, loader } = root
...
T(PackageIo).query({ '*.enable': true }, (name, item) => {
   ...
   apps[name] = app
   if (root[name]) {
      throw new Error(`root.${name}存在命名冲突，请尝试对${name}组件重新命名`)
   } else {
      root[name] = app
   }
   ...
   if (_path) {
      ...
      app.path = _path
      app.scopePath = path.join(cwd, _path, 'node_modules', '@app')
   ...
   // npm组件模块
      ...
      app.scopePath = path.join(cwd, app.path, 'node_modules', '@app')
