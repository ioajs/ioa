...
function loader(loadOptions) {
   ...
   const Lloader = require('D:/Nodejs/Project/lloader');
   const ioa = require('..');
   ...
   Lloader.loadAll(loaders, function (group) {
      ...
      for (const name in group) {
         ...
         console.log(`\x1b[32m-------------------------------- level \x1b[33m${name}\x1b[32m --------------------------------\x1b[30m`);
         const list = group[name];

         for (const item of list) {

            const { nodePath, nodeName, rootContainer, } = item;

            console.log(`\x1b[35m${nodePath}\x1b[30m \x1b[33mapps.${rootContainer.$name}.${nodeName}\x1b[30m`);
