...
不同于其它框架的是ioa更像一个应用装载器，它可以在隔离环境下装载多个应用，从而避免资源冲突。通过分级装载策略实现装载生命周期内的任意阶段载入指定模块。

支持多应用、单应用、npm组件三种模式，在三种模式下均可获得一致的代码结构和开发体验，避免产生额外的学习和迁移成本。开发者可以很轻松的在多种应用模式间切换，进行应用分离、应用整合或将应用封装为npm组件进行发布等。
...
* 支持模块分级装载，可动态调整模块的装载顺序，实现框架全生命周期管理

* 支持多点和单点两种应用模式，框架根据目录结构自动进行模式匹配

* 通过依赖配置文件进行资源互访，实现强大的资源隔离与共享机制
...
ioa中引入了目录和模块装载等级的概念，用于管理应用启动阶段的生命周期。
