...
组件化、声明式的Node.js应用加载器。

不同于常规的Web框架，ioa.js仅仅是一个应用分级加载器。遵循按需引入原则，其核心功能足够的轻量，且不集成任何与http相关的服务。

基于水平加载策略，因此开发者可以自由的管理应用启动阶段的生命周期，而不必受限于常规框架的种种约束。

ioa.js中的组件由多个平行应用构成，每个组件都拥有独立的模块作用域，从而避免资源冲突。通过声明式的依赖关系配置模型，实现跨多个应用共享组件资源。
...
* 支持组件依赖注入和多级复用，组件之间通过自由组合，可合成新的组件
...
以下目录结构仅为约定，ioa框架本身并不限制目录结构。虽然开发者通过.loader.js文件可以自由定义每个应用的目录结构，但是只有遵循统一的约定，才能更好的实现组件资源共享。
...
由于ioa使用模块分级装载策略来管理生命周期，这使得你可以在框架装载的任意阶段创建无限个平级或上下级装载点，自由管理装载流程。
