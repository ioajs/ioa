...
支持单应用、多应用、npm组件三种模式，在三种模式下均可获得一致的代码结构、同步的生命周期和开发体验，避免产生额外的学习和迁移成本。开发者可以很轻松的在多种应用模式间切换，进行应用分离、应用整合或将应用封装为npm组件进行发布等。

ioa框架并不只局限于提供web服务，可以根据需求来决定是否启用http服务，在不启用http服务的状态下可以当作应用装载器来使用。
...
* 支持单点和多点两种应用模式，框架根据目录结构自动进行模式匹配
...
在多点应用模式下以main作为默认应用目录，框架会自动为每个子应用注入独立的模块作用域，实现资源隔离，同时通过.import.js配置文件进行资源共享。
...
单节点和npm包模式都只有一个应用节点，且目录、代码结构完全一致。该模式通常用于功能模块较少的中小型项目，同时也可以作为npm组件进行发布。
