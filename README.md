...
基于组件化、声明式的全新Node.js & Koa框架。

ioa中的组件由多个平级应用构成，每个应用都拥有独立的模块作用域，通过声明式的依赖配置模型实现跨应用资源共享，同时具备常规单体应用的所有功能和一致的代码结构，包括对controller、router的支持。

不同于其它框架的是ioa更像是一个框架装载器，可以在隔离环境下装载多个应用，通过分级策略实现在装载生命周期内的任意阶段执行特定模块的代码。
...
* 支持多点应用和单点应用两种组件模式，框架根据目录结构自动进行模式匹配

* 通过io依赖配置文件进行资源互访，实现强大的资源隔离与共享机制

* 应用间使用只读的单向数据流传递依赖，减少跨组件污染

* 支持使用npm发布组件，管理组件间的版本和依赖关系
...
在多点应用模式下，框架会为每个子应用注入独立的模块作用域，实现资源隔离，同时通过io.js配置文件进行资源共享。
...
单节点模式即只有一个中心节点的常规应用，在该模式下可以通过npm来发布和管理应用的版本和依赖，可供多点应用使用或直接驱动小型项目。
...
ioa中引入了装载等级的概念，这使得你可以在框架装载的每个载阶段创建无限个平级或上下级装载点，自由控制装载流程。
